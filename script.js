...
function createCards(parentElement, shuffledImageClasses) {
  ...
  /* Step 2: Write a for loop that loops 12 times to create the 12 cards we need. */
    ...
    var imageClass = shuffledImageClasses[i];
    /* Step 2(c): Append a new object to the card object array. The object should contain the following properties:
      ...
      cardElement.classList.add(customCard.imageClass)
...
incrementCounter('matches', document.getElementById('match-count'));
  lastCardFlipped.element.classList.add('border-glow', 'match');
    newlyFlippedCard.element.classList.add('border-glow', 'match');
...
while (cardsElement.firstElementChild) { 
  cardsElement.removeChild(cardsElement.firstElementChild);
  
}
	
	/* Step 3: Get the HTML elements that display the flip and match counts and reset their inner text to 0. */ 
  let flipsElement = document.getElementById('flip-count');
  let matchElement = document.getElementById('match-count');
  
  if(flipsElement && matchElement) {
    flipsElement.textContent = 0;
    matchElement.textContent = 0;
  }
